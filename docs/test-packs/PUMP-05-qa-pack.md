...
| S-A-06 | §12-05A-6 | `/comms/create` | Sign in as a PUMP org-admin with `create:page.comms-log`. Visit `/comms/create`. | Heading "Compose", subtitle, breadcrumb, back link, sender-identity banner, recipient-mode card with default `'org_members'`, and composer render. | - | |
